...
type AgentAction =
  ...
  | {
      ...
      payload: {
        agentId: string;
        update: (prev: AgentUIState) => AgentUIState;
      };
    }
  | {
      type: "SET_DOCUMENTS";
      payload: {
        agentId: string;
        update: (prev: DocumentMetadata[]) => DocumentMetadata[];
      };
    }
  | {
      type: "SET_ROLES";
      payload: { agentId: string; update: (prev: Role[]) => Role[] };
    }
  | {
      type: "SET_ROLE";
      payload: {
    ...
    case "SET_DOCUMENTS":
      return state.map((agent) => {
        if (agent.id === action.payload.agentId) {
          return {
            ...agent,
            documents: action.payload.update(agent.documents || []),
          } as AgentUIState;
        }
        return agent;
      });
    case "SET_ROLES":
      return state.map((agent) => {
        if (agent.id === action.payload.agentId) {
          return {
            ...agent,
            roles: action.payload.update((agent as any).roles || []),
          } as AgentUIState;
        }
        return agent;
      });
